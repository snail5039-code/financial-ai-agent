...
- 실제 금융 데이터, 계좌, 주문, 체결, 시세, 환율, AI 실행, 운영 DB는 연결하지 않는다. 예외: 기업 상세 화면의 공시는 2026-09-01부터 OpenDART 공개 API에 실제로 연결한다(읽기 전용, 계좌·금액 무관). 자세한 내용은 `docs/handoff/01-current-state.md`, `05-safety-validation.md`를 참고한다.
