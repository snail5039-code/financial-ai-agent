...
1차 백엔드는 아직 생성하지 않았으며, 만들 때도 실제 DB 없이 시작한다. 현재 `apps/web` 프론트는 로컬 fixture 모듈로 대시보드, 승인 대기, 승인 전 근거 패킷 데이터를 표시한다.
...
- 금액, 수익률, 비중은 화면 표시 문자열과 원본 숫자 값을 같이 내려준다.
...
| `GET` | `/api/dashboard` | 대시보드 총자산, 차트, 보유 종목 |
