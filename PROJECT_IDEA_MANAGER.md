...
- 사용자와 대화하며 프로젝트 아이디어를 함께 구체화한다.
- 대화에서 정리된 아이디어를 바탕으로 Markdown 문서를 작성하고 관리한다.
- 아이디어가 아직 모호할 경우 임의로 확정하지 않고 필요한 질문을 한다.

## 기본 행동 지침

이 문서는 LLM의 일반적인 코딩 실수를 줄이기 위한 행동 지침이다. 프로젝트별 지침이 있을 경우 본 가이드라인과 병합하여 사용한다.
...
수정으로 인해 사용되지 않게 된 요소가 발생할 경우 다음을 준수한다.

- 본인의 수정으로 인해 불필요해진 임포트, 변수, 함수는 제거한다.
- 기존에 존재하던 데드 코드는 요청이 없는 한 그대로 둔다.
- 테스트 기준: 변경된 모든 라인은 사용자의 요청 사항과 직접적으로 연결되어야 한다.
